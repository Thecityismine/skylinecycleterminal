...
// â”€â”€ FRED fetch helper â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Signal card component (inline) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
export default async function SPXRecessionPage() {
  // Fetch all FRED series in parallel â€” SP500 from 1996 to warm up 1000d MA
  ...
  // â”€â”€ Build SPX chart data â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€ Latest signal values â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€ Recession risk score â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€ Signal interpretations â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  const ycScore  = scoreYieldCurve(latestT10y2y);
  const yieldCurveStatus = latestT10y2y > 1   ? { label: 'Normal â€” Healthy', color: '#35D07F' }
    : latestT10y2y > 0.5                       ? { label: 'Flattening', color: '#E6B450' }
    : latestT10y2y > 0                         ? { label: 'Near Flat â€” Caution', color: '#F97316' }
    : latestT10y2y > -0.5                      ? { label: 'Inverted â€” Warning', color: '#FF5C5C' }
    :                                            { label: 'Deeply Inverted', color: '#FF5C5C' };

  const sahmStatus = latestSahm >= 0.50 ? { label: 'Triggered âš ï¸Ž', color: '#FF5C5C' }
  ...
  const hyStatus = latestHY < 3.5  ? { label: 'Tight â€” Risk-On', color: '#35D07F' }
    : latestHY < 5.0               ? { label: 'Normal', color: '#E6B450' }
    : latestHY < 7.0               ? { label: 'Widening â€” Stress', color: '#F97316' }
    :                                { label: 'Distressed', color: '#FF5C5C' };

  const unrateDelta = latestUnrate - unrate12mAgo;
  const unrateStatus = unrateDelta > 0.5  ? { label: 'Rising â€” Caution', color: '#F97316' }
  ...
  const ismStatus = latestISM > 55  ? { label: 'Strong Expansion', color: '#35D07F' }
    ...
    : latestISM > 48                ? { label: 'Contraction â€” Mild', color: '#E6B450' }
    : latestISM > 45                ? { label: 'Contraction', color: '#F97316' }
    :                                 { label: 'Deep Contraction', color: '#FF5C5C' };

  const spxTrendStatus = last200w && spxPrice > last200w * 1.10 ? { label: 'Above 200W â€” Strong', color: '#35D07F' }
  ...
  return (
    <div className="max-w-[1400px] mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <StatCard
          label="S&P 500"
          value={spxPrice > 0 ? spxPrice.toLocaleString('en-US', { maximumFractionDigits: 0 }) : 'â€”'}
          sub="Latest close"
          accent="var(--sct-text)"
          freshness="daily"
        />
        <StatCard
          label="vs 200W MA"
          value={pctVs200w != null ? fmtPct(pctVs200w) : 'â€”'}
          sub={last200w ? `200W MA: ${last200w.toLocaleString('en-US', { maximumFractionDigits: 0 })}` : 'â€”'}
        ...
        <StatCard
          ...
          value={`${riskScore} / 100`}
          sub={regimeMeta.label.split('â€”')[0].trim()}
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          ...
          <SignalCard
            ...
            sub="Trigger at â‰¥ 0.50 â€” 3-month avg unemployment rise above 12-month low"
          />
          <SignalCard
            ...
            sub="ICE BofA US HY OAS â€” widening signals deteriorating credit conditions"
          />
          <SignalCard
            ...
            sub={`12-month change: ${fmtPct(unrateDelta)} Â· 12m ago: ${unrate12mAgo.toFixed(1)}%`}
          />
          <SignalCard
            label={`ISM Manufacturing${latestISMDate ? ` Â· ${latestISMDate.slice(0, 7)}` : ''}`}
            value={latestISM.toFixed(1)}
            status={ismStatus.label}
            statusColor={ismStatus.color}
            sub="Above 50 = expansion Â· below 48 = meaningful contraction"
          />
          <SignalCard
            label="SPX Trend"
            value={pctVs200w != null ? fmtPct(pctVs200w) + ' vs 200W' : 'â€”'}
      ...
      >
        <p className="text-sm font-semibold mb-4" style={{ color: 'var(--sct-text)' }}>
          Recession Risk Score Breakdown â€” {riskScore} / 100
        </p>
            ...
            return (
              <div key={label}>
                <div className="flex justify-between items-center mb-1 text-xs font-mono" style={{ color: 'var(--sct-muted)' }}>
                  <span>{label}</span>
                  <span style={{ color }}>{score} pts Â· contributes {contrib.toFixed(1)}</span>
                </div>
        ...
        <div className="flex flex-wrap gap-x-6 gap-y-1 mt-4 pt-4 border-t text-xs font-mono" style={{ borderColor: 'var(--sct-border)', color: 'var(--sct-muted)' }}>
          {[['0â€“24', 'Low Risk', '#35D07F'], ['25â€“49', 'Watch', '#E6B450'], ['50â€“74', 'Elevated', '#F97316'], ['75â€“100', 'Severe', '#FF5C5C']].map(([range, label, color]) => (
            ...
            <tbody>
              ...
              {athDrawdown < -1 && (
                <tr style={{ backgroundColor: 'var(--sct-card)', borderTop: `1px solid var(--sct-border)` }}>
                  ...
                  <td className="px-5 py-3" style={{ color: 'var(--sct-muted)' }}>â€”</td>
                  <td className="px-5 py-3" style={{ color: 'var(--sct-muted)' }}>â€”</td>
                </tr>
              )}
              <tr style={{ backgroundColor: 'var(--sct-panel)', borderTop: `1px solid var(--sct-border)` }}>
                ...
                <td className="px-5 py-3" style={{ color: 'var(--sct-muted)' }}>âˆ’46.5%</td>
      ...
      >
        ...
        <p className="text-xs leading-relaxed" style={{ color: 'var(--sct-muted)' }}>
          ...
          This page measures recession risk and historical context â€” <strong style={{ color: 'var(--sct-text)' }}>not a prediction of the next recession.</strong>
      ...
      <InsightPanel title="Signal Definitions">
        <InsightRow label="Yield Curve (10Yâˆ’2Y)" value="Spread between 10-year and 2-year Treasury yields. Inversion (negative) has preceded every US recession since 1955. The key signal is re-steepening after inversion â€” especially when unemployment is also rising." stack />
        <InsightRow label="Sahm Rule" value="Triggers when the 3-month average unemployment rate rises â‰¥ 0.50 percentage points above its 12-month low. Created by Fed economist Claudia Sahm. Has correctly identified every recession in real time since 1970." valueColor="#E6B450" stack />
        <InsightRow label="HY Credit Spreads" value="ICE BofA US High Yield OAS (BAMLH0A0HYM2). Credit spreads widen when markets price in higher default risk. Typically widened 200â€“600 bps before and during recessions." stack />
        <InsightRow label="ISM Manufacturing PMI" value="Institute for Supply Management Manufacturing Index (NAPM on FRED). Above 50 = expansion. A sustained move below 48 has historically signaled broader economic weakness within 2â€“4 quarters." stack />
        <InsightRow label="Source" value="S&P 500: FRED SP500 Â· Yield Curve: FRED T10Y2Y Â· Sahm Rule: FRED SAHMREALTIME Â· HY Spread: FRED BAMLH0A0HYM2 Â· Unemployment: FRED UNRATE Â· ISM: FRED NAPM" stack />
      </InsightPanel>
    </div>
  );
}
