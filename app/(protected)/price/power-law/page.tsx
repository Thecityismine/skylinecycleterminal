...
const ZONE_META = {
  above_ceil:  { label: 'Above Ceiling — Extreme Overvaluation', color: '#F472B6',
    desc: 'Price has broken above the historical ceiling band. Every prior occurrence marked a major cycle top within months.' },
  above_fair:  { label: 'Above Fair Value — Elevated', color: '#F97316',
    desc: 'Price is above the power law median. Historically the second half of bull markets. Risk/reward deteriorating — reduce exposure on strength.' },
  below_fair:  { label: 'Below Fair Value — Accumulation Zone', color: '#38BDF8',
  ...
  below_floor: { label: 'Below Floor — Deep Value / Bear Market', color: '#818CF8',
  ...
  return (
    <div className="max-w-[1400px] mx-auto space-y-6">
      <PageHeader
        ...
        subtitle="Price follows a power law relationship with time — log₁₀(P) = 5.82 × log₁₀(days) − 16.73"
      />
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <StatCard
          ...
          sub={stats ? fmtPct(stats.pctVsFair) + ' vs fair' : '—'}
          accent="#38BDF8"
        />
        <StatCard
          label="Floor"
          value={fmtUSD(stats?.floor ?? null)}
          sub={stats ? `Lead: ${fmtYrs(stats.leadFloor)}` : '—'}
          accent="#818CF8"
        />
        <StatCard
          label="Ceiling"
          value={fmtUSD(stats?.ceil ?? null)}
          sub={stats ? `Lead: ${fmtYrs(stats.leadCeil)}` : '—'}
          accent="#F472B6"
        />
      </div>
      ...
      <InsightPanel title="Model Overview">
        <InsightRow
          ...
          value="log₁₀(P) = 5.82 × log₁₀(D) − 16.73, where D = days since genesis block (2009-01-03). On a log-log chart, price forms a straight line — suggesting BTC's growth is deterministic, not random."
          stack
        />
        <InsightRow
        ...
        <InsightRow
          label="Floor (×0.42)"
          value="Historical lower bound — fair value × 0.42. Bear market bottoms (2015, 2018, 2022) tested but rarely breached this level. The strongest historical buy signals occur here."
          valueColor="#818CF8"
          stack
        />
        <InsightRow
          label="Ceiling (×4.27)"
          value="Historical upper bound — fair value × 4.27. Each bull market cycle top has approached or briefly exceeded this level before reversing."
          valueColor="#F472B6"
          stack
        />
        <InsightRow
          label="Lead vs Floor / Ceiling"
          value="Years until the floor or ceiling reaches the current BTC price. A positive floor lead means BTC is above where the floor will be — tracking how much support is building beneath."
          stack
        />
        <InsightRow
          label="Source"
          value="Model: Giovanni Santostasi / MCO Legacy Power Law · Price data: CoinMetrics Community API"
          stack
        />
      </InsightPanel>
    </div>
