...
import { TwoYearMAShareCard, TWO_YEAR_MA_CARD_CHART_RECT } from '@/components/share/TwoYearMAShareCard';
import type { TwoYearMASharePayload } from '@/components/share/TwoYearMAShareCard';
import {
  exportShareCard,
  compositeWatermark,
  downloadPng,
...
type Props = { payload: TwoYearMASharePayload };
type ExportState = 'idle' | 'exporting' | 'ready';

const PREVIEW_SCALE = 0.42;

export function TwoYearMAShareModal({ payload }: Props) {
...
function Modal({ payload, onClose }: { payload: TwoYearMASharePayload; onClose: () => void }) {
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      const url = await compositeWatermark(raw, logoSrc ?? '', TWO_YEAR_MA_CARD_CHART_RECT);
      setDataUrl(url);
      setState('ready');
    } catch {
      setState('idle');
  ...
  const handleDownload = useCallback(() => {
    if (!dataUrl) return;
    downloadPng(dataUrl, `skyline-2yr-ma-${Date.now()}.png`);
  }, [dataUrl]);

  const handleShare = useCallback(async () => {
    if (!dataUrl) return;
    const file = dataUrlToFile(dataUrl, 'skyline-2yr-ma.png');
    const ok = await shareImageFile(file, 'Skyline Cycle Terminal — 2-Year Moving Average');
      ...
      >
        ...
        <div className="relative p-4 flex items-center justify-center" style={{ backgroundColor: '#090D13' }}>
          <div style={{
            ...
            <div ref={cardRef}>
              <TwoYearMAShareCard payload={payload} />
            </div>
          </div>
