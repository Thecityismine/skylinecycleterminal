...
import type { CalendarSharePayload } from '@/components/share/CalendarShareCard';
import {
  exportShareCard,
  compositeWatermark,
  downloadPng,
...
export function CalendarShareModal({ payload, onClose }: Props) {
  ...
  const handleDownload = useCallback(() => {
    if (!dataUrl) return;
    downloadPng(dataUrl, `skyline-crypto-calendar-${Date.now()}.png`);
  }, [dataUrl]);

  const handleShare = useCallback(async () => {
    if (!dataUrl) return;
    const file = dataUrlToFile(dataUrl, 'skyline-crypto-calendar.png');
    const ok = await shareImageFile(file, 'Skyline Crypto Calendar');
      ...
      >
        ...
        <div className="relative p-4 flex items-center justify-center" style={{ backgroundColor: '#090D13' }}>
          <div style={{
            ...
            <div ref={cardRef}>
              <CalendarShareCard payload={payload} />
            </div>
          </div>
