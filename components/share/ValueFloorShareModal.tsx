...
import { ValueFloorShareCard, VALUE_FLOOR_CARD_CHART_RECT } from '@/components/share/ValueFloorShareCard';
...
type Props = { payload: ValueFloorSharePayload };
type ExportState = 'idle' | 'exporting' | 'ready';

const PREVIEW_SCALE = 0.42;

export function ValueFloorShareModal({ payload }: Props) {
...
function Modal({ payload, onClose }: { payload: ValueFloorSharePayload; onClose: () => void }) {
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      const url = await compositeWatermark(raw, logoSrc ?? '', VALUE_FLOOR_CARD_CHART_RECT);
      setDataUrl(url);
      setState('ready');
    } catch {
      setState('idle');
  ...
  const handleDownload = useCallback(() => {
    if (!dataUrl) return;
    downloadPng(dataUrl, `skyline-value-floors-${Date.now()}.png`);
  }, [dataUrl]);

  const handleShare = useCallback(async () => {
    if (!dataUrl) return;
    const file = dataUrlToFile(dataUrl, 'skyline-value-floors.png');
    const ok = await shareImageFile(file, 'Skyline Cycle Terminal — Bitcoin Value Floors');
      ...
      >
        ...
        <div className="relative p-4 flex items-center justify-center" style={{ backgroundColor: '#090D13' }}>
          <div style={{
            ...
            <div ref={cardRef}>
              <ValueFloorShareCard payload={payload} />
            </div>
          </div>
