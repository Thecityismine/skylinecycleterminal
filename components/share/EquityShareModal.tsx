...
import { EquityShareCard, EQUITY_CARD_CHART_RECT } from '@/components/share/EquityShareCard';
...
type Props = { payload: EquitySharePayload };
type ExportState = 'idle' | 'exporting' | 'ready';

const PREVIEW_SCALE = 0.42;

export function EquityShareModal({ payload }: Props) {
...
function Modal({ payload, onClose }: { payload: EquitySharePayload; onClose: () => void }) {
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      const url = await compositeWatermark(raw, logoSrc ?? '', EQUITY_CARD_CHART_RECT);
      setDataUrl(url);
      setState('ready');
    } catch {
      setState('idle');
  ...
  const handleDownload = useCallback(() => {
    if (!dataUrl) return;
    downloadPng(dataUrl, `skyline-${payload.ticker.toLowerCase()}-${Date.now()}.png`);
  }, [dataUrl, payload.ticker]);

  const handleShare = useCallback(async () => {
    if (!dataUrl) return;
    const file = dataUrlToFile(dataUrl, `skyline-${payload.ticker.toLowerCase()}.png`);
    const ok = await shareImageFile(file, `Skyline Cycle Terminal — ${payload.ticker}`);
    if (!ok) handleDownload();
  }, [dataUrl, payload.ticker, handleDownload]);
    ...
    >
      ...
      >
        <div className="flex items-center justify-between px-4 py-3" style={{ borderBottom: '1px solid #21262D' }}>
          <p className="text-sm font-semibold" style={{ color: '#E6EDF3' }}>Share Chart Card — {payload.ticker}</p>
        ...
        <div className="relative p-4 flex items-center justify-center" style={{ backgroundColor: '#090D13' }}>
          <div style={{
            ...
            <div ref={cardRef}>
              <EquityShareCard payload={payload} />
            </div>
          </div>
