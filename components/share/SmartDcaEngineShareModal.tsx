...
import { SmartDcaEngineShareCard, SMART_DCA_CARD_CHART_RECT } from '@/components/share/SmartDcaEngineShareCard';
...
type Props = { payload: SmartDcaEngineSharePayload };
type ExportState = 'idle' | 'exporting' | 'ready';

const PREVIEW_SCALE = 0.42;

export function SmartDcaEngineShareModal({ payload }: Props) {
...
function Modal({ payload, onClose }: { payload: SmartDcaEngineSharePayload; onClose: () => void }) {
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      const url = await compositeWatermark(raw, logoSrc ?? '', SMART_DCA_CARD_CHART_RECT);
      setDataUrl(url);
      setState('ready');
    } catch {
      setState('idle');
  ...
  const handleDownload = useCallback(() => {
    if (!dataUrl) return;
    downloadPng(dataUrl, `skyline-smart-dca-${Date.now()}.png`);
  }, [dataUrl]);

  const handleShare = useCallback(async () => {
    if (!dataUrl) return;
    const file = dataUrlToFile(dataUrl, 'skyline-smart-dca.png');
    const ok = await shareImageFile(file, 'Skyline Cycle Terminal — Smart DCA Engine');
      ...
      >
        ...
        <div className="relative p-4 flex items-center justify-center" style={{ backgroundColor: '#090D13' }}>
          <div style={{
            ...
            <div ref={cardRef}>
              <SmartDcaEngineShareCard payload={payload} />
            </div>
          </div>
