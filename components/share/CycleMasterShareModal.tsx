...
import { CycleMasterShareCard, CYCLE_MASTER_CARD_CHART_RECT } from '@/components/share/CycleMasterShareCard';
...
type Props = { payload: CycleMasterSharePayload };
type ExportState = 'idle' | 'exporting' | 'ready';

const PREVIEW_SCALE = 0.42;

export function CycleMasterShareModal({ payload }: Props) {
...
function Modal({ payload, onClose }: { payload: CycleMasterSharePayload; onClose: () => void }) {
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      const url = await compositeWatermark(raw, logoSrc ?? '', CYCLE_MASTER_CARD_CHART_RECT);
      setDataUrl(url);
      setState('ready');
    } catch {
      setState('idle');
  ...
  const handleDownload = useCallback(() => {
    if (!dataUrl) return;
    downloadPng(dataUrl, `skyline-cycle-master-${Date.now()}.png`);
  }, [dataUrl]);

  const handleShare = useCallback(async () => {
    if (!dataUrl) return;
    const file = dataUrlToFile(dataUrl, 'skyline-cycle-master.png');
    const ok = await shareImageFile(file, 'Skyline Cycle Terminal — Bitcoin Cycle Master');
      ...
      >
        ...
        <div className="relative p-4 flex items-center justify-center" style={{ backgroundColor: '#090D13' }}>
          <div style={{
            ...
            <div ref={cardRef}>
              <CycleMasterShareCard payload={payload} />
            </div>
          </div>
