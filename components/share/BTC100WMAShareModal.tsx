...
import { BTC100WMAShareCard, BTC_100W_MA_CARD_CHART_RECT } from '@/components/share/BTC100WMAShareCard';
...
type Props = { payload: BTC100WMASharePayload };
type ExportState = 'idle' | 'exporting' | 'ready';

const PREVIEW_SCALE = 0.42;

export function BTC100WMAShareModal({ payload }: Props) {
...
function Modal({ payload, onClose }: { payload: BTC100WMASharePayload; onClose: () => void }) {
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      const url = await compositeWatermark(raw, logoSrc ?? '', BTC_100W_MA_CARD_CHART_RECT);
      setDataUrl(url);
      setState('ready');
    } catch {
      setState('idle');
  ...
  const handleDownload = useCallback(() => {
    if (!dataUrl) return;
    downloadPng(dataUrl, `skyline-100w-ma-${Date.now()}.png`);
  }, [dataUrl]);

  const handleShare = useCallback(async () => {
    if (!dataUrl) return;
    const file = dataUrlToFile(dataUrl, 'skyline-100w-ma.png');
    const ok = await shareImageFile(file, 'Skyline Cycle Terminal — Bitcoin 100-Week MA');
      ...
      >
        ...
        <div className="relative p-4 flex items-center justify-center" style={{ backgroundColor: '#090D13' }}>
          <div style={{
            ...
            <div ref={cardRef}>
              <BTC100WMAShareCard payload={payload} />
            </div>
          </div>
