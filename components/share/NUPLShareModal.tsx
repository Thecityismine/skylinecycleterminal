...
import { NUPLShareCard, NUPL_CARD_CHART_RECT } from '@/components/share/NUPLShareCard';
...
type Props = { payload: NUPLSharePayload };
type ExportState = 'idle' | 'exporting' | 'ready';

const PREVIEW_SCALE = 0.42;

export function NUPLShareModal({ payload }: Props) {
...
function Modal({ payload, onClose }: { payload: NUPLSharePayload; onClose: () => void }) {
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      const url = await compositeWatermark(raw, logoSrc ?? '', NUPL_CARD_CHART_RECT);
      setDataUrl(url);
      setState('ready');
    } catch {
      setState('idle');
  ...
  const handleDownload = useCallback(() => {
    if (!dataUrl) return;
    downloadPng(dataUrl, `skyline-nupl-${Date.now()}.png`);
  }, [dataUrl]);

  const handleShare = useCallback(async () => {
    if (!dataUrl) return;
    const file = dataUrlToFile(dataUrl, 'skyline-nupl.png');
    const ok = await shareImageFile(file, 'Skyline Cycle Terminal — NUPL');
