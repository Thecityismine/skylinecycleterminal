...
import { SPXShareCard, SPX_CARD_CHART_RECT } from '@/components/share/SPXShareCard';
...
type Props = { payload: SPXSharePayload };
type ExportState = 'idle' | 'exporting' | 'ready';

const PREVIEW_SCALE = 0.42;

export function SPXShareModal({ payload }: Props) {
...
function Modal({ payload, onClose }: { payload: SPXSharePayload; onClose: () => void }) {
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      const url = await compositeWatermark(raw, logoSrc ?? '', SPX_CARD_CHART_RECT);
      setDataUrl(url);
      setState('ready');
    } catch {
      setState('idle');
  ...
  const handleDownload = useCallback(() => {
    if (!dataUrl) return;
    downloadPng(dataUrl, `skyline-spx-${Date.now()}.png`);
  }, [dataUrl]);

  const handleShare = useCallback(async () => {
    if (!dataUrl) return;
    const file = dataUrlToFile(dataUrl, 'skyline-spx-chart.png');
    const ok = await shareImageFile(file, 'Skyline Cycle Terminal — S&P 500 & Recession Risk');
