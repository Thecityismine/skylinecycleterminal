...
import { DrawdownShareCard, DRAWDOWN_CARD_CHART_RECT } from '@/components/share/DrawdownShareCard';
...
type Props = { payload: DrawdownSharePayload };
type ExportState = 'idle' | 'exporting' | 'ready';

const PREVIEW_SCALE = 0.42;

export function DrawdownShareModal({ payload }: Props) {
...
function Modal({ payload, onClose }: { payload: DrawdownSharePayload; onClose: () => void }) {
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      const url = await compositeWatermark(raw, logoSrc ?? '', DRAWDOWN_CARD_CHART_RECT);
      setDataUrl(url);
      setState('ready');
    } catch {
      setState('idle');
  ...
  const handleDownload = useCallback(() => {
    if (!dataUrl) return;
    downloadPng(dataUrl, `skyline-drawdown-${Date.now()}.png`);
  }, [dataUrl]);

  const handleShare = useCallback(async () => {
    if (!dataUrl) return;
    const file = dataUrlToFile(dataUrl, 'skyline-drawdown.png');
    const ok = await shareImageFile(file, 'Skyline Cycle Terminal — Bitcoin Drawdown From ATH');
      ...
      >
        ...
        <div className="relative p-4 flex items-center justify-center" style={{ backgroundColor: '#090D13' }}>
          <div style={{
            ...
            <div ref={cardRef}>
              <DrawdownShareCard payload={payload} />
            </div>
          </div>
