...
type Props = { payload: FourYearCycleSharePayload };

type ExportState = 'idle' | 'exporting' | 'ready';

const PREVIEW_SCALE = 0.42;

export function FourYearCycleShareModal({ payload }: Props) {
...
function Modal({ payload, onClose }: { payload: FourYearCycleSharePayload; onClose: () => void }) {
  ...
  const handleDownload = useCallback(() => {
    if (!dataUrl) return;
    downloadPng(dataUrl, `skyline-4yr-cycle-${Date.now()}.png`);
  }, [dataUrl]);

  const handleShare = useCallback(async () => {
    if (!dataUrl) return;
    const file = dataUrlToFile(dataUrl, 'skyline-4yr-cycle.png');
    const ok = await shareImageFile(file, 'Skyline Cycle Terminal — Bitcoin 4-Year Cycle');
      ...
      >
        ...
        <div className="relative p-4 flex items-center justify-center" style={{ backgroundColor: '#090D13' }}>
          <div style={{
            ...
            <div ref={cardRef}>
              <FourYearCycleShareCard payload={payload} />
            </div>
          </div>
