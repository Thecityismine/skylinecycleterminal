...
import { ValuationDeviationShareCard, VALUATION_DEVIATION_CARD_CHART_RECT } from '@/components/share/ValuationDeviationShareCard';
...
type Props = { payload: ValuationDeviationSharePayload };
type ExportState = 'idle' | 'exporting' | 'ready';

const PREVIEW_SCALE = 0.42;

export function ValuationDeviationShareModal({ payload }: Props) {
...
function Modal({ payload, onClose }: { payload: ValuationDeviationSharePayload; onClose: () => void }) {
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      const url = await compositeWatermark(raw, logoSrc ?? '', VALUATION_DEVIATION_CARD_CHART_RECT);
      setDataUrl(url);
      setState('ready');
    } catch {
      setState('idle');
  ...
  const handleDownload = useCallback(() => {
    if (!dataUrl) return;
    downloadPng(dataUrl, `skyline-cycle-valuation-${Date.now()}.png`);
  }, [dataUrl]);

  const handleShare = useCallback(async () => {
    if (!dataUrl) return;
    const file = dataUrlToFile(dataUrl, 'skyline-cycle-valuation.png');
    const ok = await shareImageFile(file, 'Skyline Cycle Terminal — Bitcoin Cycle Valuation');
      ...
      >
        ...
        <div className="relative p-4 flex items-center justify-center" style={{ backgroundColor: '#090D13' }}>
          <div style={{
            ...
            <div ref={cardRef}>
              <ValuationDeviationShareCard payload={payload} />
            </div>
          </div>
