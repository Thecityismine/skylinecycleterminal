...
import { BTCGLIShareCard, BTC_GLI_CARD_CHART_RECT } from '@/components/share/BTCGLIShareCard';
...
type Props = { payload: BTCGLISharePayload; onClose: () => void };
type ExportState = 'idle' | 'exporting' | 'ready';

const PREVIEW_SCALE = 0.42;

export function BTCGLIShareModal({ payload, onClose }: Props) {
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      const url = await compositeWatermark(raw, logoSrc ?? '', BTC_GLI_CARD_CHART_RECT);
      setDataUrl(url);
      setState('ready');
    } catch {
      setState('idle');
  ...
  const handleDownload = useCallback(() => {
    if (!dataUrl) return;
    downloadPng(dataUrl, `skyline-btc-gli-lag-${Date.now()}.png`);
  }, [dataUrl]);

  const handleShare = useCallback(async () => {
    if (!dataUrl) return;
    const file = dataUrlToFile(dataUrl, 'skyline-btc-gli-lag.png');
    const ok = await shareImageFile(file, 'Skyline Cycle Terminal — BTC vs GLI Liquidity Lag');
      ...
      >
        ...
        <div className="relative p-4 flex items-center justify-center" style={{ backgroundColor: '#090D13' }}>
          <div style={{
            ...
            <div ref={cardRef}>
              <BTCGLIShareCard payload={payload} />
            </div>
          </div>
