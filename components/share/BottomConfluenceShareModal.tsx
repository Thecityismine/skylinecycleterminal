...
import { BottomConfluenceShareCard, BOTTOM_CONFLUENCE_CARD_CHART_RECT } from '@/components/share/BottomConfluenceShareCard';
...
type Props = { payload: BottomConfluenceSharePayload };
type ExportState = 'idle' | 'exporting' | 'ready';

const PREVIEW_SCALE = 0.42;

export function BottomConfluenceShareModal({ payload }: Props) {
...
function Modal({ payload, onClose }: { payload: BottomConfluenceSharePayload; onClose: () => void }) {
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      const url = await compositeWatermark(raw, logoSrc ?? '', BOTTOM_CONFLUENCE_CARD_CHART_RECT);
      setDataUrl(url);
      setState('ready');
    } catch {
      setState('idle');
  ...
  const handleDownload = useCallback(() => {
    if (!dataUrl) return;
    downloadPng(dataUrl, `skyline-bottom-confluence-${Date.now()}.png`);
  }, [dataUrl]);

  const handleShare = useCallback(async () => {
    if (!dataUrl) return;
    const file = dataUrlToFile(dataUrl, 'skyline-bottom-confluence.png');
    const ok = await shareImageFile(file, 'Skyline Cycle Terminal — Bottom Confluence');
      ...
      >
        ...
        <div className="relative p-4 flex items-center justify-center" style={{ backgroundColor: '#090D13' }}>
          <div style={{
            ...
            <div ref={cardRef}>
              <BottomConfluenceShareCard payload={payload} />
            </div>
          </div>
