...
import { HalvingWindowsShareCard, HALVING_WINDOWS_CARD_CHART_RECT } from '@/components/share/HalvingWindowsShareCard';
import type { HalvingWindowsSharePayload } from '@/components/share/HalvingWindowsShareCard';
import {
  exportShareCard,
  compositeWatermark,
  downloadPng,
...
type Props = {
  payload: HalvingWindowsSharePayload;
  onClose: () => void;
};
...
export function HalvingWindowsShareModal({ payload, onClose }: Props) {
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      const url = await compositeWatermark(raw, logoSrc ?? '', HALVING_WINDOWS_CARD_CHART_RECT);
      setDataUrl(url);
      setState('ready');
    } catch {
      setState('idle');
  ...
  const handleDownload = useCallback(() => {
    if (!dataUrl) return;
    downloadPng(dataUrl, `skyline-halving-windows-${payload.rangeLabel.toLowerCase().replace(/\s+/g, '-')}-${Date.now()}.png`);
  }, [dataUrl, payload.rangeLabel]);

  const handleShare = useCallback(async () => {
    if (!dataUrl) return;
    const file = dataUrlToFile(dataUrl, 'skyline-halving-windows.png');
    const ok = await shareImageFile(file, 'Skyline Cycle Terminal — BTC Halving Cycle Windows');
      ...
      >
        ...
        <div className="relative p-4 flex items-center justify-center" style={{ backgroundColor: '#090D13' }}>
          ...
          <div style={{
            ...
            <div ref={cardRef}>
              <HalvingWindowsShareCard payload={payload} />
            </div>
          </div>
