...
import { SevenYearCycleShareCard, SEVEN_YEAR_CARD_CHART_RECT } from '@/components/share/SevenYearCycleShareCard';
...
type Props = { payload: SevenYearCycleSharePayload };
type ExportState = 'idle' | 'exporting' | 'ready';

const PREVIEW_SCALE = 0.42;

export function SevenYearCycleShareModal({ payload }: Props) {
...
function Modal({ payload, onClose }: { payload: SevenYearCycleSharePayload; onClose: () => void }) {
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      const url = await compositeWatermark(raw, logoSrc ?? '', SEVEN_YEAR_CARD_CHART_RECT);
      setDataUrl(url);
      setState('ready');
    } catch {
      setState('idle');
  ...
  const handleDownload = useCallback(() => {
    if (!dataUrl) return;
    downloadPng(dataUrl, `skyline-seven-year-cycle-${Date.now()}.png`);
  }, [dataUrl]);

  const handleShare = useCallback(async () => {
    if (!dataUrl) return;
    const file = dataUrlToFile(dataUrl, 'skyline-seven-year-cycle.png');
    const ok = await shareImageFile(file, 'Skyline Cycle Terminal — Bitcoin 7-Year Stress Cycle');
      ...
      >
        ...
        <div className="relative p-4 flex items-center justify-center" style={{ backgroundColor: '#090D13' }}>
          <div style={{
            ...
            <div ref={cardRef}>
              <SevenYearCycleShareCard payload={payload} />
            </div>
          </div>
