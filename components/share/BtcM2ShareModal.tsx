...
import { BtcM2ShareCard, BTC_M2_CARD_CHART_RECT } from '@/components/share/BtcM2ShareCard';
...
type Props = { payload: BtcM2SharePayload };
type ExportState = 'idle' | 'exporting' | 'ready';

const PREVIEW_SCALE = 0.42;

export function BtcM2ShareModal({ payload }: Props) {
...
function Modal({ payload, onClose }: { payload: BtcM2SharePayload; onClose: () => void }) {
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      const url = await compositeWatermark(raw, logoSrc ?? '', BTC_M2_CARD_CHART_RECT);
      setDataUrl(url);
      setState('ready');
    } catch {
      setState('idle');
  ...
  const handleDownload = useCallback(() => {
    if (!dataUrl) return;
    downloadPng(dataUrl, `skyline-btc-m2-${Date.now()}.png`);
  }, [dataUrl]);

  const handleShare = useCallback(async () => {
    if (!dataUrl) return;
    const file = dataUrlToFile(dataUrl, 'skyline-btc-m2.png');
    const ok = await shareImageFile(file, 'Skyline Cycle Terminal — BTC / M2 Money Supply');
      ...
      >
        ...
        <div className="relative p-4 flex items-center justify-center" style={{ backgroundColor: '#090D13' }}>
          <div style={{
            ...
            <div ref={cardRef}>
              <BtcM2ShareCard payload={payload} />
            </div>
          </div>
