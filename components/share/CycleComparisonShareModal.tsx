...
import { CycleComparisonShareCard, CYCLE_COMPARISON_CARD_CHART_RECT } from '@/components/share/CycleComparisonShareCard';
...
type Props = {
  payload: CycleComparisonSharePayload;
  onClose: () => void;
};
...
export function CycleComparisonShareModal({ payload, onClose }: Props) {
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      const url = await compositeWatermark(raw, logoSrc ?? '', CYCLE_COMPARISON_CARD_CHART_RECT);
      setDataUrl(url);
      setState('ready');
    } catch {
      setState('idle');
  ...
  const handleDownload = useCallback(() => {
    if (!dataUrl) return;
    downloadPng(dataUrl, `skyline-cycle-comparison-${Date.now()}.png`);
  }, [dataUrl]);

  const handleShare = useCallback(async () => {
    if (!dataUrl) return;
    const file = dataUrlToFile(dataUrl, 'skyline-cycle-comparison.png');
    const ok = await shareImageFile(file, 'Skyline Cycle Terminal — Bitcoin Cycle Comparison');
      ...
      >
        ...
        <div className="relative p-4 flex items-center justify-center" style={{ backgroundColor: '#090D13' }}>
          ...
          <div style={{
            ...
            <div ref={cardRef}>
              <CycleComparisonShareCard payload={payload} />
            </div>
          </div>
