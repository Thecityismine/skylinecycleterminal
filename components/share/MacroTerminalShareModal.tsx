...
import { MacroTerminalShareCard, MACRO_TERMINAL_CARD_CHART_RECT } from '@/components/share/MacroTerminalShareCard';
import type { MacroTerminalSharePayload } from '@/components/share/MacroTerminalShareCard';
import {
  exportShareCard,
  compositeWatermark,
  downloadPng,
...
type Props = { payload: MacroTerminalSharePayload };
...
let webShareSupport: boolean | null = null;
function canWebShareFiles(): boolean {
  if (webShareSupport === null) {
  ...
  return webShareSupport;
}

export function MacroTerminalShareModal({ payload }: Props) {
...
function Modal({ payload, onClose }: { payload: MacroTerminalSharePayload; onClose: () => void }) {
  ...
  // Web Share support differs between server and client, so it is read through
  // useSyncExternalStore with a `false` server snapshot rather than set from an
  // effect. The subscribe callback is a no-op: the answer cannot change at runtime.
  const hasShare = useSyncExternalStore(subscribeNever, canWebShareFiles, () => false);
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      const url = await compositeWatermark(raw, logoSrc ?? '', MACRO_TERMINAL_CARD_CHART_RECT);
      setDataUrl(url);
      setState('ready');
    } catch {
      setState('idle');
  ...
  // Wait for the logo to resolve, then let the off-screen card paint before capture.
  useEffect(() => {
  ...
  const handleDownload = useCallback(() => {
    if (!dataUrl) return;
    downloadPng(dataUrl, `skyline-macro-terminal-${Date.now()}.png`);
  }, [dataUrl]);

  const handleShare = useCallback(async () => {
    if (!dataUrl) return;
    const file = dataUrlToFile(dataUrl, 'skyline-macro-terminal.png');
    const ok = await shareImageFile(file, 'Skyline Macro Terminal: Macro Risk Score');
    ...
    >
      ...
      >
        <div className="flex items-center justify-between px-4 py-3" style={{ borderBottom: '1px solid #21262D' }}>
          <p className="text-sm font-semibold" style={{ color: '#E6EDF3' }}>Share Macro Risk Card</p>
        ...
        <div className="relative p-4 flex items-center justify-center" style={{ backgroundColor: '#090D13' }}>
          <div style={{
            ...
            <div ref={cardRef}>
              <MacroTerminalShareCard payload={payload} />
            </div>
          </div>
