...
import { LiquidityRegimeShareCard, LIQUIDITY_REGIME_CARD_CHART_RECT } from '@/components/share/LiquidityRegimeShareCard';
...
type Props = { payload: LiquidityRegimeSharePayload; onClose: () => void };
type ExportState = 'idle' | 'exporting' | 'ready';

const PREVIEW_SCALE = 0.42;

export function LiquidityRegimeShareModal({ payload, onClose }: Props) {
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      const url = await compositeWatermark(raw, logoSrc ?? '', LIQUIDITY_REGIME_CARD_CHART_RECT);
      setDataUrl(url);
      setState('ready');
    } catch {
      setState('idle');
  ...
  const handleDownload = useCallback(() => {
    if (!dataUrl) return;
    downloadPng(dataUrl, `skyline-liquidity-regime-${Date.now()}.png`);
  }, [dataUrl]);

  const handleShare = useCallback(async () => {
    if (!dataUrl) return;
    const file = dataUrlToFile(dataUrl, 'skyline-liquidity-regime.png');
    const ok = await shareImageFile(file, 'Skyline Cycle Terminal — BTC Liquidity Regime Matrix');
      ...
      >
        ...
        <div className="relative p-4 flex items-center justify-center" style={{ backgroundColor: '#090D13' }}>
          <div style={{
            ...
            <div ref={cardRef}>
              <LiquidityRegimeShareCard payload={payload} />
            </div>
          </div>
