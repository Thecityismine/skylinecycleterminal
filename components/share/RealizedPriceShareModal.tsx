...
import { RealizedPriceShareCard, REALIZED_PRICE_CARD_CHART_RECT } from '@/components/share/RealizedPriceShareCard';
...
type Props = { payload: RealizedPriceSharePayload };
type ExportState = 'idle' | 'exporting' | 'ready';

const PREVIEW_SCALE = 0.42;

export function RealizedPriceShareModal({ payload }: Props) {
...
function Modal({ payload, onClose }: { payload: RealizedPriceSharePayload; onClose: () => void }) {
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      const url = await compositeWatermark(raw, logoSrc ?? '', REALIZED_PRICE_CARD_CHART_RECT);
      setDataUrl(url);
      setState('ready');
    } catch {
      setState('idle');
  ...
  const handleDownload = useCallback(() => {
    if (!dataUrl) return;
    downloadPng(dataUrl, `skyline-realized-price-${Date.now()}.png`);
  }, [dataUrl]);

  const handleShare = useCallback(async () => {
    if (!dataUrl) return;
    const file = dataUrlToFile(dataUrl, 'skyline-realized-price.png');
    const ok = await shareImageFile(file, 'Skyline Cycle Terminal — BTC vs 200-Week MA');
      ...
      >
        ...
        <div className="relative p-4 flex items-center justify-center" style={{ backgroundColor: '#090D13' }}>
          <div style={{
            ...
            <div ref={cardRef}>
              <RealizedPriceShareCard payload={payload} />
            </div>
          </div>
