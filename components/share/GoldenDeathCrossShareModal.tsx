...
import { GoldenDeathCrossShareCard, GOLDEN_DEATH_CROSS_CARD_CHART_RECT } from '@/components/share/GoldenDeathCrossShareCard';
import type { GoldenDeathCrossSharePayload } from '@/components/share/GoldenDeathCrossShareCard';
import {
  exportShareCard,
  compositeWatermark,
  downloadPng,
...
type Props = {
  payload: GoldenDeathCrossSharePayload;
  onClose: () => void;
};
...
export function GoldenDeathCrossShareModal({ payload, onClose }: Props) {
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      const url = await compositeWatermark(raw, logoSrc ?? '', GOLDEN_DEATH_CROSS_CARD_CHART_RECT);
      setDataUrl(url);
      setState('ready');
    } catch {
      setState('idle');
  ...
  const handleDownload = useCallback(() => {
    if (!dataUrl) return;
    downloadPng(dataUrl, `skyline-golden-death-cross-${payload.rangeLabel.toLowerCase().replace(/\s+/g, '-')}-${Date.now()}.png`);
  }, [dataUrl, payload.rangeLabel]);

  const handleShare = useCallback(async () => {
    if (!dataUrl) return;
    const file = dataUrlToFile(dataUrl, 'skyline-golden-death-cross.png');
    const ok = await shareImageFile(file, 'Skyline Cycle Terminal — BTC Golden / Death Cross');
      ...
      >
        ...
        <div className="relative p-4 flex items-center justify-center" style={{ backgroundColor: '#090D13' }}>
          ...
          <div style={{
            ...
            <div ref={cardRef}>
              <GoldenDeathCrossShareCard payload={payload} />
            </div>
          </div>
