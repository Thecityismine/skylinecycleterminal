...
import { PiCycleShareCard, PI_CYCLE_CARD_CHART_RECT } from '@/components/share/PiCycleShareCard';
...
type Props = { payload: PiCycleSharePayload };
type ExportState = 'idle' | 'exporting' | 'ready';

const PREVIEW_SCALE = 0.42;

export function PiCycleShareModal({ payload }: Props) {
...
function Modal({ payload, onClose }: { payload: PiCycleSharePayload; onClose: () => void }) {
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      const url = await compositeWatermark(raw, logoSrc ?? '', PI_CYCLE_CARD_CHART_RECT);
      setDataUrl(url);
      setState('ready');
    } catch {
      setState('idle');
  ...
  const handleDownload = useCallback(() => {
    if (!dataUrl) return;
    downloadPng(dataUrl, `skyline-pi-cycle-bottom-${Date.now()}.png`);
  }, [dataUrl]);

  const handleShare = useCallback(async () => {
    if (!dataUrl) return;
    const file = dataUrlToFile(dataUrl, 'skyline-pi-cycle-bottom.png');
    const ok = await shareImageFile(file, 'Skyline Cycle Terminal — Pi Cycle Bottom');
      ...
      >
        ...
        <div className="relative p-4 flex items-center justify-center" style={{ backgroundColor: '#090D13' }}>
          <div style={{
            ...
            <div ref={cardRef}>
              <PiCycleShareCard payload={payload} />
            </div>
          </div>
