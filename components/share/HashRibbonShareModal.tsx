...
import { HashRibbonShareCard, HASH_RIBBON_CARD_CHART_RECT } from '@/components/share/HashRibbonShareCard';
...
type Props = { payload: HashRibbonSharePayload };
type ExportState = 'idle' | 'exporting' | 'ready';

const PREVIEW_SCALE = 0.42;

export function HashRibbonShareModal({ payload }: Props) {
...
function Modal({ payload, onClose }: { payload: HashRibbonSharePayload; onClose: () => void }) {
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      const url = await compositeWatermark(raw, logoSrc ?? '', HASH_RIBBON_CARD_CHART_RECT);
      setDataUrl(url);
      setState('ready');
    } catch {
      setState('idle');
  ...
  const handleDownload = useCallback(() => {
    if (!dataUrl) return;
    downloadPng(dataUrl, `skyline-hash-ribbons-${Date.now()}.png`);
  }, [dataUrl]);

  const handleShare = useCallback(async () => {
    if (!dataUrl) return;
    const file = dataUrlToFile(dataUrl, 'skyline-hash-ribbons.png');
    const ok = await shareImageFile(file, 'Skyline Cycle Terminal — Hash Ribbons');
      ...
      >
        ...
        <div className="relative p-4 flex items-center justify-center" style={{ backgroundColor: '#090D13' }}>
          <div style={{
            ...
            <div ref={cardRef}>
              <HashRibbonShareCard payload={payload} />
            </div>
          </div>
