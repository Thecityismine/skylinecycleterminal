...
import { PowerLawShareCard, POWER_LAW_CARD_CHART_RECT } from '@/components/share/PowerLawShareCard';
...
type Props = { payload: PowerLawSharePayload };
type ExportState = 'idle' | 'exporting' | 'ready';

const PREVIEW_SCALE = 0.42;

export function PowerLawShareModal({ payload }: Props) {
...
function Modal({ payload, onClose }: { payload: PowerLawSharePayload; onClose: () => void }) {
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      const url = await compositeWatermark(raw, logoSrc ?? '', POWER_LAW_CARD_CHART_RECT);
      setDataUrl(url);
      setState('ready');
    } catch {
      setState('idle');
  ...
  const handleDownload = useCallback(() => {
    if (!dataUrl) return;
    downloadPng(dataUrl, `skyline-power-law-${Date.now()}.png`);
  }, [dataUrl]);

  const handleShare = useCallback(async () => {
    if (!dataUrl) return;
    const file = dataUrlToFile(dataUrl, 'skyline-power-law.png');
    const ok = await shareImageFile(file, 'Skyline Cycle Terminal — Bitcoin Power Law');
      ...
      >
        ...
        <div className="relative p-4 flex items-center justify-center" style={{ backgroundColor: '#090D13' }}>
          <div style={{
            ...
            <div ref={cardRef}>
              <PowerLawShareCard payload={payload} />
            </div>
          </div>
