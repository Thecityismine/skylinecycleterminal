...
import { RatioShareCard, RATIO_CARD_CHART_RECT } from '@/components/share/RatioShareCard';
import type { RatioSharePayload } from '@/components/share/RatioShareCard';
import {
  exportShareCard,
  compositeWatermark,
  downloadPng,
...
type Props = { payload: RatioSharePayload };
type ExportState = 'idle' | 'exporting' | 'ready';

const PREVIEW_SCALE = 0.42;

export function RatioShareModal({ payload }: Props) {
...
function Modal({ payload, onClose }: { payload: RatioSharePayload; onClose: () => void }) {
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      const url = await compositeWatermark(raw, logoSrc ?? '', RATIO_CARD_CHART_RECT);
      setDataUrl(url);
      setState('ready');
    } catch {
      setState('idle');
  ...
  const handleDownload = useCallback(() => {
    if (!dataUrl) return;
    const slug = payload.ratioKey.replace('_', '-');
    downloadPng(dataUrl, `skyline-${slug}-${Date.now()}.png`);
  }, [dataUrl, payload.ratioKey]);

  const handleShare = useCallback(async () => {
    if (!dataUrl) return;
    const file = dataUrlToFile(dataUrl, `skyline-${payload.ratioKey}.png`);
    const ok = await shareImageFile(file, `Skyline Cycle Terminal — ${payload.ratioLabel} Ratio`);
    if (!ok) handleDownload();
  }, [dataUrl, payload.ratioKey, payload.ratioLabel, handleDownload]);
      ...
      >
        ...
        <div className="relative p-4 flex items-center justify-center" style={{ backgroundColor: '#090D13' }}>
          <div style={{
            ...
            <div ref={cardRef}>
              <RatioShareCard payload={payload} />
            </div>
          </div>
