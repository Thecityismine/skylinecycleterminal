...
import { WeeklySMAShareCard, WEEKLY_SMA_CARD_CHART_RECT } from '@/components/share/WeeklySMAShareCard';
...
type Props = { payload: WeeklySMASharePayload };
type ExportState = 'idle' | 'exporting' | 'ready';

const PREVIEW_SCALE = 0.42;

export function WeeklySMAShareModal({ payload }: Props) {
...
function Modal({ payload, onClose }: { payload: WeeklySMASharePayload; onClose: () => void }) {
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      const url = await compositeWatermark(raw, logoSrc ?? '', WEEKLY_SMA_CARD_CHART_RECT);
      setDataUrl(url);
      setState('ready');
    } catch {
      setState('idle');
  ...
  const handleDownload = useCallback(() => {
    if (!dataUrl) return;
    downloadPng(dataUrl, `skyline-weekly-sma-${payload.asset}-${Date.now()}.png`);
  }, [dataUrl, payload.asset]);

  const handleShare = useCallback(async () => {
    if (!dataUrl) return;
    const file = dataUrlToFile(dataUrl, `skyline-weekly-sma-${payload.asset}.png`);
    const ok = await shareImageFile(file, `Skyline Cycle Terminal — ${payload.assetLabel} Weekly SMA`);
    if (!ok) handleDownload();
  }, [dataUrl, payload.asset, payload.assetLabel, handleDownload]);
      ...
      >
        ...
        <div className="relative p-4 flex items-center justify-center" style={{ backgroundColor: '#090D13' }}>
          <div style={{
            ...
            <div ref={cardRef}>
              <WeeklySMAShareCard payload={payload} />
            </div>
          </div>
