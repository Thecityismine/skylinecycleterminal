...
import { RoadToOneMillionShareCard, ROAD_TO_1M_CARD_CHART_RECT } from '@/components/share/RoadToOneMillionShareCard';
import type { RoadToOneMillionSharePayload } from '@/components/share/RoadToOneMillionShareCard';
import {
  exportShareCard,
  compositeWatermark,
  downloadPng,
...
type Props = { payload: RoadToOneMillionSharePayload; disabled?: boolean };
type ExportState = 'idle' | 'exporting' | 'ready';

const PREVIEW_SCALE = 0.42;

export function RoadToOneMillionShareModal({ payload, disabled }: Props) {
  ...
  return (
    <>
      <button
        ...
        disabled={disabled}
        className="flex items-center gap-1.5 rounded-md px-2.5 py-1 text-xs font-mono border transition-all duration-150"
        style={{
...
function Modal({ payload, onClose }: { payload: RoadToOneMillionSharePayload; onClose: () => void }) {
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      const url = await compositeWatermark(raw, logoSrc ?? '', ROAD_TO_1M_CARD_CHART_RECT);
      setDataUrl(url);
      setState('ready');
    } catch {
      setState('idle');
  ...
  const handleDownload = useCallback(() => {
    if (!dataUrl) return;
    downloadPng(dataUrl, `skyline-road-to-1m-${Date.now()}.png`);
  }, [dataUrl]);

  const handleShare = useCallback(async () => {
    if (!dataUrl) return;
    const file = dataUrlToFile(dataUrl, 'skyline-road-to-1m.png');
    const ok = await shareImageFile(file, 'Skyline Cycle Terminal — Road to $1M');
      ...
      >
        ...
        <div className="relative p-4 flex items-center justify-center" style={{ backgroundColor: '#090D13' }}>
          <div style={{
            ...
            <div ref={cardRef}>
              <RoadToOneMillionShareCard payload={payload} />
            </div>
          </div>
