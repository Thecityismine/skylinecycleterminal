...
import { SoprShareCard, SOPR_CARD_CHART_RECT } from '@/components/share/SoprShareCard';
...
type Props = { payload: SoprSharePayload };
type ExportState = 'idle' | 'exporting' | 'ready';

const PREVIEW_SCALE = 0.42;

export function SoprShareModal({ payload }: Props) {
...
function Modal({ payload, onClose }: { payload: SoprSharePayload; onClose: () => void }) {
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      const url = await compositeWatermark(raw, logoSrc ?? '', SOPR_CARD_CHART_RECT);
      setDataUrl(url);
      setState('ready');
    } catch {
      setState('idle');
  ...
  const handleDownload = useCallback(() => {
    if (!dataUrl) return;
    downloadPng(dataUrl, `skyline-sopr-${Date.now()}.png`);
  }, [dataUrl]);

  const handleShare = useCallback(async () => {
    if (!dataUrl) return;
    const file = dataUrlToFile(dataUrl, 'skyline-sopr.png');
    const ok = await shareImageFile(file, 'Skyline Cycle Terminal — Bitcoin SOPR');
