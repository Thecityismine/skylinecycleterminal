...
import { StablecoinDominanceShareCard, STABLECOIN_DOM_CARD_CHART_RECT } from '@/components/share/StablecoinDominanceShareCard';
...
type Props = { payload: StablecoinDominanceSharePayload };
type ExportState = 'idle' | 'exporting' | 'ready';

const PREVIEW_SCALE = 0.42;

export function StablecoinDominanceShareModal({ payload }: Props) {
...
function Modal({ payload, onClose }: { payload: StablecoinDominanceSharePayload; onClose: () => void }) {
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      const url = await compositeWatermark(raw, logoSrc ?? '', STABLECOIN_DOM_CARD_CHART_RECT);
      setDataUrl(url);
      setState('ready');
    } catch {
      setState('idle');
  ...
  const handleDownload = useCallback(() => {
    if (!dataUrl) return;
    downloadPng(dataUrl, `skyline-stablecoin-dominance-${Date.now()}.png`);
  }, [dataUrl]);

  const handleShare = useCallback(async () => {
    if (!dataUrl) return;
    const file = dataUrlToFile(dataUrl, 'skyline-stablecoin-dominance.png');
    const ok = await shareImageFile(file, 'Skyline Cycle Terminal — Stablecoin Dominance');
    ...
    >
      ...
      >
        <div className="flex items-center justify-between px-4 py-3" style={{ borderBottom: '1px solid #21262D' }}>
          <p className="text-sm font-semibold" style={{ color: '#E6EDF3' }}>Share Chart Card — Stablecoin Dominance</p>
        ...
        <div className="relative p-4 flex items-center justify-center" style={{ backgroundColor: '#090D13' }}>
          <div style={{
            ...
            <div ref={cardRef}>
              <StablecoinDominanceShareCard payload={payload} />
            </div>
          </div>
        ...
        <div className="flex items-center gap-2 px-4 py-3" style={{ borderTop: '1px solid #21262D' }}>
          ...
          <button
            ...
            style={{
              backgroundColor: dataUrl ? '#4DA3FF' : '#21262D',
