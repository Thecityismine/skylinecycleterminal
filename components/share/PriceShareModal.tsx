...
export function PriceShareModal({ payload, onClose }: Props) {
  ...
  const handleDownload = useCallback(() => {
    if (!dataUrl) return;
    const label = `${payload.asset}-${payload.timeframe.toLowerCase()}`;
    downloadPng(dataUrl, `skyline-price-${label}-${Date.now()}.png`);
  }, [dataUrl, payload.asset, payload.timeframe]);

  const handleShare = useCallback(async () => {
    if (!dataUrl) return;
    const file = dataUrlToFile(dataUrl, `skyline-price-${payload.asset}.png`);
    const ok = await shareImageFile(file, `Skyline Cycle Terminal — ${payload.asset.toUpperCase()} Price`);
    if (!ok) handleDownload();
  }, [dataUrl, payload.asset, handleDownload]);
