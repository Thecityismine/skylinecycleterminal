...
import { BTCDCAOptimizerShareCard, DCA_OPTIMIZER_CARD_CHART_RECT } from '@/components/share/BTCDCAOptimizerShareCard';
...
type Props = { payload: BTCDCAOptimizerSharePayload };
type ExportState = 'idle' | 'exporting' | 'ready';

const PREVIEW_SCALE = 0.42;

export function BTCDCAOptimizerShareModal({ payload }: Props) {
...
function Modal({ payload, onClose }: { payload: BTCDCAOptimizerSharePayload; onClose: () => void }) {
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      const url = await compositeWatermark(raw, logoSrc ?? '', DCA_OPTIMIZER_CARD_CHART_RECT);
      setDataUrl(url);
      setState('ready');
    } catch {
      setState('idle');
  ...
  const handleDownload = useCallback(() => {
    if (!dataUrl) return;
    downloadPng(dataUrl, `skyline-dca-optimizer-${Date.now()}.png`);
  }, [dataUrl]);

  const handleShare = useCallback(async () => {
    if (!dataUrl) return;
    const file = dataUrlToFile(dataUrl, 'skyline-dca-optimizer.png');
    const ok = await shareImageFile(file, 'Skyline Cycle Terminal — BTC DCA Optimizer');
      ...
      >
        ...
        <div className="relative p-4 flex items-center justify-center" style={{ backgroundColor: '#090D13' }}>
          <div style={{
            ...
            <div ref={cardRef}>
              <BTCDCAOptimizerShareCard payload={payload} />
            </div>
          </div>
