...
import { RotationShareCard, ROTATION_CARD_CHART_RECT } from '@/components/share/RotationShareCard';
import type { RotationSharePayload } from '@/components/share/RotationShareCard';
import {
...
type Props = { tabKey: string; payload: RotationSharePayload; onClose: () => void };
type ExportState = 'idle' | 'exporting' | 'ready';

const PREVIEW_SCALE = 0.42;

export function RotationShareModal({ tabKey, payload, onClose }: Props) {
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      const url = await compositeWatermark(raw, logoSrc ?? '', ROTATION_CARD_CHART_RECT);
      setDataUrl(url);
      setState('ready');
    } catch {
      setState('idle');
  ...
  const handleDownload = useCallback(() => {
    if (!dataUrl) return;
    downloadPng(dataUrl, `skyline-market-rotation-${tabKey}-${Date.now()}.png`);
  }, [dataUrl, tabKey]);

  const handleShare = useCallback(async () => {
    if (!dataUrl) return;
    const file = dataUrlToFile(dataUrl, `skyline-market-rotation-${tabKey}.png`);
    const ok = await shareImageFile(file, `Skyline Cycle Terminal — ${payload.title}`);
    if (!ok) handleDownload();
  }, [dataUrl, tabKey, payload.title, handleDownload]);
      ...
      >
        ...
        <div className="relative p-4 flex items-center justify-center" style={{ backgroundColor: '#090D13' }}>
          <div style={{
            ...
            <div ref={cardRef}>
              <RotationShareCard payload={payload} />
            </div>
          </div>
