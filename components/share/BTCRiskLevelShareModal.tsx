...
import { BTCRiskLevelShareCard, RISK_LEVEL_CARD_CHART_RECT } from '@/components/share/BTCRiskLevelShareCard';
...
type Props = { payload: BTCRiskLevelSharePayload };
type ExportState = 'idle' | 'exporting' | 'ready';

const PREVIEW_SCALE = 0.42;

export function BTCRiskLevelShareModal({ payload }: Props) {
...
function Modal({ payload, onClose }: { payload: BTCRiskLevelSharePayload; onClose: () => void }) {
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      const url = await compositeWatermark(raw, logoSrc ?? '', RISK_LEVEL_CARD_CHART_RECT);
      setDataUrl(url);
      setState('ready');
    } catch {
      setState('idle');
  ...
  const handleDownload = useCallback(() => {
    if (!dataUrl) return;
    downloadPng(dataUrl, `skyline-risk-level-${Date.now()}.png`);
  }, [dataUrl]);

  const handleShare = useCallback(async () => {
    if (!dataUrl) return;
    const file = dataUrlToFile(dataUrl, 'skyline-risk-level.png');
    const ok = await shareImageFile(file, 'Skyline Cycle Terminal — BTC Historical Risk Level');
      ...
      >
        ...
        <div className="relative p-4 flex items-center justify-center" style={{ backgroundColor: '#090D13' }}>
          <div style={{
            ...
            <div ref={cardRef}>
              <BTCRiskLevelShareCard payload={payload} />
            </div>
          </div>
