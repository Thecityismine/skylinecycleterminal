...
import { GenerationZoneShareCard, GENERATION_ZONE_CARD_CHART_RECT } from '@/components/share/GenerationZoneShareCard';
import type { GenerationZoneSharePayload } from '@/components/share/GenerationZoneShareCard';
import {
  exportShareCard,
  compositeWatermark,
  downloadPng,
...
type Props = { payload: GenerationZoneSharePayload };
type ExportState = 'idle' | 'exporting' | 'ready';

const PREVIEW_SCALE = 0.42;

export function GenerationZoneShareModal({ payload }: Props) {
...
function Modal({ payload, onClose }: { payload: GenerationZoneSharePayload; onClose: () => void }) {
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      const url = await compositeWatermark(raw, logoSrc ?? '', GENERATION_ZONE_CARD_CHART_RECT);
      setDataUrl(url);
      setState('ready');
    } catch {
      setState('idle');
  ...
  const handleDownload = useCallback(() => {
    if (!dataUrl) return;
    downloadPng(dataUrl, `skyline-generation-zone-${Date.now()}.png`);
  }, [dataUrl]);

  const handleShare = useCallback(async () => {
    if (!dataUrl) return;
    const file = dataUrlToFile(dataUrl, 'skyline-generation-zone.png');
    const ok = await shareImageFile(file, 'Skyline Cycle Terminal — Generation Buying Zone');
      ...
      >
        ...
        <div className="relative p-4 flex items-center justify-center" style={{ backgroundColor: '#090D13' }}>
          <div style={{
            ...
            <div ref={cardRef}>
              <GenerationZoneShareCard payload={payload} />
            </div>
          </div>
