...
import { HousingCycleShareCard, HOUSING_CYCLE_CARD_CHART_RECT } from '@/components/share/HousingCycleShareCard';
import type { HousingCycleSharePayload } from '@/components/share/HousingCycleShareCard';
import {
  exportShareCard,
  compositeWatermark,
  downloadPng,
...
type Props = { payload: HousingCycleSharePayload };
type ExportState = 'idle' | 'exporting' | 'ready';

const PREVIEW_SCALE = 0.42;

export function HousingCycleShareModal({ payload }: Props) {
...
function Modal({ payload, onClose }: { payload: HousingCycleSharePayload; onClose: () => void }) {
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      const url = await compositeWatermark(raw, logoSrc ?? '', HOUSING_CYCLE_CARD_CHART_RECT);
      setDataUrl(url);
      setState('ready');
    } catch {
      setState('idle');
  ...
  const handleDownload = useCallback(() => {
    if (!dataUrl) return;
    downloadPng(dataUrl, `skyline-housing-cycle-${Date.now()}.png`);
  }, [dataUrl]);

  const handleShare = useCallback(async () => {
    if (!dataUrl) return;
    const file = dataUrlToFile(dataUrl, 'skyline-housing-cycle.png');
    const ok = await shareImageFile(file, 'Skyline Cycle Terminal — The Long Housing Cycle');
      ...
      >
        ...
        <div className="relative p-4 flex items-center justify-center" style={{ backgroundColor: '#090D13' }}>
          ...
          <div style={{
            ...
            <div ref={cardRef}>
              <HousingCycleShareCard payload={payload} />
            </div>
          </div>
