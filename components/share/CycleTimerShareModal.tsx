...
import { CycleTimerShareCard, CYCLE_TIMER_CARD_CHART_RECT } from '@/components/share/CycleTimerShareCard';
...
type Props = { payload: CycleTimerSharePayload };
type ExportState = 'idle' | 'exporting' | 'ready';

const PREVIEW_SCALE = 0.42;

export function CycleTimerShareModal({ payload }: Props) {
...
function Modal({ payload, onClose }: { payload: CycleTimerSharePayload; onClose: () => void }) {
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      const url = await compositeWatermark(raw, logoSrc ?? '', CYCLE_TIMER_CARD_CHART_RECT);
      setDataUrl(url);
      setState('ready');
    } catch {
      setState('idle');
  ...
  const handleDownload = useCallback(() => {
    if (!dataUrl) return;
    downloadPng(dataUrl, `skyline-cycle-timer-${Date.now()}.png`);
  }, [dataUrl]);

  const handleShare = useCallback(async () => {
    if (!dataUrl) return;
    const file = dataUrlToFile(dataUrl, 'skyline-cycle-timer.png');
    const ok = await shareImageFile(file, 'Skyline Cycle Terminal — Bitcoin Cycle Duration Timer');
      ...
      >
        ...
        <div className="relative p-4 flex items-center justify-center" style={{ backgroundColor: '#090D13' }}>
          <div style={{
            ...
            <div ref={cardRef}>
              <CycleTimerShareCard payload={payload} />
            </div>
          </div>
