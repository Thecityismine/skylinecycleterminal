...
import { HodlWaveShareCard, HODL_WAVE_CARD_CHART_RECT } from '@/components/share/HodlWaveShareCard';
...
type Props = { payload: HodlWaveSharePayload };
type ExportState = 'idle' | 'exporting' | 'ready';

const PREVIEW_SCALE = 0.42;

export function HodlWaveShareModal({ payload }: Props) {
...
function Modal({ payload, onClose }: { payload: HodlWaveSharePayload; onClose: () => void }) {
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      const url = await compositeWatermark(raw, logoSrc ?? '', HODL_WAVE_CARD_CHART_RECT);
      setDataUrl(url);
      setState('ready');
    } catch {
      setState('idle');
  ...
  const handleDownload = useCallback(() => {
    if (!dataUrl) return;
    downloadPng(dataUrl, `skyline-hodl-wave-${Date.now()}.png`);
  }, [dataUrl]);

  const handleShare = useCallback(async () => {
    if (!dataUrl) return;
    const file = dataUrlToFile(dataUrl, 'skyline-hodl-wave.png');
    const ok = await shareImageFile(file, 'Skyline Cycle Terminal — Bitcoin HODL Wave');
      ...
      >
        ...
        <div className="relative p-4 flex items-center justify-center" style={{ backgroundColor: '#090D13' }}>
          <div style={{
            ...
            <div ref={cardRef}>
              <HodlWaveShareCard payload={payload} />
            </div>
          </div>
