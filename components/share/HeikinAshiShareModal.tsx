...
import { HeikinAshiShareCard, HEIKIN_ASHI_CARD_CHART_RECT } from '@/components/share/HeikinAshiShareCard';
...
type Props = { payload: HeikinAshiSharePayload };
type ExportState = 'idle' | 'exporting' | 'ready';

const PREVIEW_SCALE = 0.42;

export function HeikinAshiShareModal({ payload }: Props) {
...
function Modal({ payload, onClose }: { payload: HeikinAshiSharePayload; onClose: () => void }) {
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      const url = await compositeWatermark(raw, logoSrc ?? '', HEIKIN_ASHI_CARD_CHART_RECT);
      setDataUrl(url);
      setState('ready');
    } catch {
      setState('idle');
  ...
  const handleDownload = useCallback(() => {
    if (!dataUrl) return;
    downloadPng(dataUrl, `skyline-heikin-ashi-${Date.now()}.png`);
  }, [dataUrl]);

  const handleShare = useCallback(async () => {
    if (!dataUrl) return;
    const file = dataUrlToFile(dataUrl, 'skyline-heikin-ashi.png');
    const ok = await shareImageFile(file, 'Skyline Cycle Terminal — Monthly Heikin-Ashi');
      ...
      >
        ...
        <div className="relative p-4 flex items-center justify-center" style={{ backgroundColor: '#090D13' }}>
          <div style={{
            ...
            <div ref={cardRef}>
              <HeikinAshiShareCard payload={payload} />
            </div>
          </div>
