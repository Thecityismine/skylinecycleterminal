...
export function RegimeShareModal({ payload, onClose }: Props) {
  ...
  const handleDownload = useCallback(() => {
    if (!dataUrl) return;
    downloadPng(dataUrl, `skyline-regime-${Date.now()}.png`);
  }, [dataUrl]);

  const handleShare = useCallback(async () => {
    if (!dataUrl) return;
    const file = dataUrlToFile(dataUrl, 'skyline-market-regime.png');
    const ok = await shareImageFile(file, 'Skyline Cycle Terminal — BTC Market Regime');
