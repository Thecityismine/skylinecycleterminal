...
import { CycleDurationShareCard, CYCLE_DURATION_CARD_CHART_RECT } from '@/components/share/CycleDurationShareCard';
...
type Props = { payload: CycleDurationSharePayload };
type ExportState = 'idle' | 'exporting' | 'ready';

const PREVIEW_SCALE = 0.42;

export function CycleDurationShareModal({ payload }: Props) {
...
function Modal({ payload, onClose }: { payload: CycleDurationSharePayload; onClose: () => void }) {
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      const url = await compositeWatermark(raw, logoSrc ?? '', CYCLE_DURATION_CARD_CHART_RECT);
      setDataUrl(url);
      setState('ready');
    } catch {
      setState('idle');
  ...
  const handleDownload = useCallback(() => {
    if (!dataUrl) return;
    downloadPng(dataUrl, `skyline-cycle-duration-${Date.now()}.png`);
  }, [dataUrl]);

  const handleShare = useCallback(async () => {
    if (!dataUrl) return;
    const file = dataUrlToFile(dataUrl, 'skyline-cycle-duration.png');
    const ok = await shareImageFile(file, 'Skyline Cycle Terminal — BTC 1,064 / 364 Cycle Model');
      ...
      >
        ...
        <div className="relative p-4 flex items-center justify-center" style={{ backgroundColor: '#090D13' }}>
          <div style={{
            ...
            <div ref={cardRef}>
              <CycleDurationShareCard payload={payload} />
            </div>
          </div>
